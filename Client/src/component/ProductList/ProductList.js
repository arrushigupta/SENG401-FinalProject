...
import {
  deleteProductRoute,
  getAllProductsRoute,
  getSpecificProductCategoryRoute,
  getSpecificProductNameRoute,
  getSpecificProductRoute,
  getSpecificProductUserIDRoute,
  getTwoSpecificProductsCategoryRoute,
} from "../../utils/Routes";
    ...
    if (chooseMessage === 0) {
      ...
      if (searchFlag != prevSearchFlag) {
        ...
        if (searchCategory == "All") {
          ...
          if (searchQuery === "") {
            apiString2 = apiString2 =
              getSpecificProductCategoryRoute + searchCategory;
          } else {
            apiString2 =
              getTwoSpecificProductsCategoryRoute +
              searchCategory +
      ...
      <div className="items-center flex-cols justify-center">
        ...
        <div className="grid w-screen sm:grid-cols:1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  ">
          {products.map((product) => (
            <div
              style={{
                display: product._id === deletedProductId ? "none" : "block",
              }}
              class="group w-fit h-fit rounded-lg bg-slate-100 hover:bg-red-600 hover:shadow-xl active:bg-red-700 m-4 md:hover:text-neutral-50"
            >
